...
**Only bump a recipe's `version` field when the change is being committed to `master`.**

- During local edits or drafting, do NOT change the version.
- When the change is ready to commit (or is already staged for master), bump the version in the same commit.
